...
if __name__ == '__main__':
  ...
  model.load_weights('src/models/bf_model_w_v2.h5')
  labels = ['back', 'front']

  files = glob.glob('./src/images/correct/*.png')
  ...
  for idx, image_filename in enumerate(files_to_use):
    ...
    pred = model.predict(img_array.reshape(1, 256, 256, 3))[0][0]
    thresholld = 0.5
    idx_pred = None
    if pred > thresholld:
      idx_pred = 1
    ...
    new_path = './src/images/' + label_pred + '/' + f'{pred:.4f}_' + name_file
